...
9. **Verification commands:** The project has two `vendor/` trees, each with its own tooling binaries. `phpunit`, `phpcs`, and `phpstan` are configured in their respective config files (`phpunit.xml.dist`, `phpcs.xml.dist`, `phpstan.neon.dist`) in the framework package, so commands can be run from either the project root or `vendor/omega-mvc/omega-wp-framework/` without needing to pass extra parameters (e.g. `--standard`). When running any of these tools, consult the config file first to check for relevant settings; if a flag is needed, pass it on the command line. **Do not modify** `phpunit.xml.dist`, `phpcs.xml.dist`, or `phpstan.neon.dist` without explicit user consent.
